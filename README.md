...
表格下方可以「編輯選取」或「刪除選取」，也支援在選取座標後按 `Delete` 刪除。雙擊一筆座標可直接修改名稱、X/Y、備註及是否使用視窗相對座標。
...
③內的「座標亮度觸發」分頁可建立獨立觸發器：

- 選擇要偵測的座標及取樣半徑
- 設定亮度門檻（0～255）
- 達到門檻時執行指定動作
- 若一直沒發亮，超過指定秒數仍強制執行（設為 0 可停用）
- 動作可組合 `F1`～`F12` 與指定座標的滑鼠左鍵／右鍵

亮度使用座標附近「最亮 25% 像素的平均值」，降低單一雜訊亮點造成的誤判。實際亮度值與觸發原因會顯示在④的事件紀錄，方便調整門檻。
在新增或編輯觸發器時，可按「測試目前亮度」立即確認目前數值是否會越過門檻。
